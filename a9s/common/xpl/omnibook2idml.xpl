...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" xmlns:c="http://www.w3.org/ns/xproc-step"
  ...
  <p:for-each name="split" cx:depends-on="image-id">
    ...
    <p:variable name="possibly-calculated-idml-target-uri" 
      select="($idml-target-uri[normalize-space()], 
               replace(base-uri(), '(/xml)?/([^/]+)\.xml$', '/idml/$2.idml', 'i'))[1]">
      <p:pipe port="source" step="omnibook2idml"/>
    </p:variable>
    ...
    <tr:xml2idml name="xml2idml">
      ...
      <p:with-option name="idml-target-uri" 
        select="replace($possibly-calculated-idml-target-uri, '([^/]+?)(_print)?\.idml', concat('$1_', $filename, '.idml'))"/>
